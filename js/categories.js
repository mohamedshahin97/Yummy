// ? =============> Global ===============>
let mealsCategoriesArray = [];
let categoryMealsArray = [];
...
async function getCategoriesMeals() {
    ...
    mealsCategoriesArray = myResponse.categories;
    // console.log(mealsCategoriesArray);
    $('#loading').hide();
    displayCategoriesMeals();
}


function displayCategoriesMeals() {
    let cartona = ``;

    for (let i = 0; i < mealsCategoriesArray.length; i++) {

        cartona += `<div class="col mt-4">
        <div onclick="getMeal('${mealsCategoriesArray[i].strCategory}')" class="mealCard rounded-2">
            <img src="${mealsCategoriesArray[i].strCategoryThumb}" class="w-100" alt="">
            <div class="mealCategoriesOverlay text-black p-2">
                <div class="text text-center">
                    <h2>${mealsCategoriesArray[i].strCategory}</h2>
                    <p>${mealsCategoriesArray[i].strCategoryDescription.split(" ").slice(0, 20).join(" ")}</p>
                </div>
            </div>
        </div>
    </div>`
    ...
    document.getElementById('rowCategoriesMeals').innerHTML = cartona;
}
...
async function getMeal(meal) {
    $('#loading').show();
    // console.log(meal);
    document.getElementById('allCategories').classList.add('d-none');
    const api = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${meal}`);
    const myResponse = await api.json();
...
function displayCategoryMeals() {
    ...
    for (let i = 0; i < categoryMealsArray.length; i++) {
        cartona += `<div class="col mt-4">
        <div onclick="displayMeal('${categoryMealsArray[i].idMeal}')" class="mealCard rounded-2">
            <img src="${categoryMealsArray[i].strMealThumb}" class="w-100" alt="">
            <div class="mealOverlay text-black p-2">
                <div class="text">
                    <h3>${categoryMealsArray[i].strMeal}</h3>
                </div>
            </div>
        </div>
    </div>`
    ...
    // console.log(categoryMealsArray);
    document.getElementById('rowCategoryMeals').innerHTML = cartona;
}
