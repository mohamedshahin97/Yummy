...
async function getIngredients() {
    ...
    ingredientsArray = myResponse.meals;
...
function displayIngredients() {  
    ...
    for (let i = 0; i < 24; i++) {
        ...
        <div onclick="getIngredientsMeals('${ingredientsArray[i].strIngredient}')" style="cursor:pointer;" id="AreaCard" class="AreaCard text-center text-white">
            <i class="fa-solid fa-drumstick-bite fa-4x"></i>
            <h3>${ingredientsArray[i].strIngredient}</h3>
            <p>${ingredientsArray[i].strDescription.split(' ').slice(0,20).join(' ')}</p>
        </div>
    </div>`
...
async function getIngredientsMeals(ingred) {
    ...
    let myResponse = await api.json();
    ingredientMealsArray = myResponse.meals;
    // console.log(ingredientMealsArray);
    $('.chooseIngredients').hide();
    document.getElementById('ingredientMeals').classList.remove('d-none');
...
function displayIngredientMeals() {  
    ...
    for (let i = 0; i < ingredientMealsArray.length; i++) {
        
        cartona += `<div class="col mt-4">
        <div onclick="displayMeal('${ingredientMealsArray[i].idMeal}')" class="mealCard rounded-2">
            <img src="${ingredientMealsArray[i].strMealThumb}" class="w-100" alt="">
    ...
    document.getElementById('rowingredientMeals').innerHTML = cartona;
}
...
async function displayMeal(mealId) {
    $('#loading').show();
    // console.log(mealId);
    document.getElementById('chooseIngredients').classList.add('d-none');
